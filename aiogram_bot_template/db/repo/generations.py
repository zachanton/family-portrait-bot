...
async def create_generation_request(db: PostgresConnection, draft: GenerationRequestDraft) -> int:
    ...
    async with db.transaction() as con:
        sql_insert_request = """
            INSERT INTO generation_requests (user_id, status, referral_source_at_creation, request_parameters)
        ...
        request_id = await con.fetchval(
            ...
            draft.request_parameters,  # <-- ИЗМЕНЕНИЕ: УБРАН json.dumps()
        )
...
async def create_generation_log(db: PostgresConnection, log_data: GenerationLog) -> int:
    ...
    sql = """
        INSERT INTO generations (
            request_id, type, status, quality_level, prompt_payload, trial_type, seed,
            result_image_unique_id, result_message_id, result_file_id, caption, 
            control_message_id, context_metadata, error_message, generation_time_ms, 
    ...
    result = await db.fetchrow(
        ...
        (
            ...
            log_data.prompt_payload, log_data.trial_type,
            log_data.seed, log_data.result_image_unique_id,
            log_data.result_message_id, log_data.result_file_id,
            log_data.caption, 
            log_data.control_message_id,
            log_data.context_metadata,
...
async def update_request_parameters(db: PostgresConnection, request_id: int, params: GenerationParameters) -> None:
    """
    Merges new parameters into the request_parameters JSON field using a type-safe DTO.
    """
    # 2. The function now expects a DTO and handles serialization internally.
    params_json = params.to_db_json()

    sql = """
        UPDATE generation_requests
        SET request_parameters = COALESCE(request_parameters::jsonb, '{}'::jsonb) || $2::jsonb
        WHERE id = $1;
    """
    await db.execute(sql, (request_id, params_json))


async def get_original_generation_type(db: PostgresConnection, request_id: int) -> str | None:
    """Fetches the 'type' of the very first generation log for a given request."""
    # We are looking for the very first generation log to find out the original type
    sql = """
        SELECT type FROM generations
        WHERE request_id = $1
        ORDER BY created_at ASC
        LIMIT 1;
    """
    result = await db.fetchrow(sql, (request_id,))
    return result.data.get("type") if result.data else None


async def get_generation_details(db: PostgresConnection, generation_id: int) -> dict | None:
    """Fetches a single generation log with all its details by its primary key."""
    sql = "SELECT * FROM generations WHERE id = $1"
    result = await db.fetchrow(sql, (generation_id,))
